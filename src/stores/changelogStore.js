...
export const useChangelogStore = defineStore('changelog', () => {
  ...
  const statistics = computed(() => {
    return {
      ...
      majorVersions: changelogs.value.filter(log => log.type === 'major').length,
      minorVersions: changelogs.value.filter(log => log.type === 'minor').length,
      patchVersions: changelogs.value.filter(log => log.type === 'patch').length,
      hotfixVersions: changelogs.value.filter(log => log.type === 'hotfix').length
    }
  })
  ...
  const getVersionDiff = (fromVersion, toVersion) => {
    const fromIndex = changelogs.value.findIndex(log => log.version === fromVersion)
    const toIndex = changelogs.value.findIndex(log => log.version === toVersion)
